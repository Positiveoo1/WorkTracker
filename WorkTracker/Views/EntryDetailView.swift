...
struct EntryDetailView: View {
    ...
    init(vm: WorkEntryViewModel, date: Date) {
        ...
        let defaultStart = cal.date(bySettingHour: 9, minute: 0, second: 0, of: dayStart) ?? Date()
        let defaultEnd = cal.date(bySettingHour: 17, minute: 0, second: 0, of: dayStart) ?? Date()
    ...
    var body: some View {
        NavigationView {
            ...
            .alert(isPresented: $showAlert) {
                Alert(title: Text("Invalid Input"), message: Text("Ensure end time is after start time and hourly rate is valid."), dismissButton: .default(Text("OK")))
            }
        }
    }
}
